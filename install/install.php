...
public \$test = array(
	...
	//'encoding' => 'utf8',
		...
		if (file_put_contents('../Config/database.php', $data)) {
		    ...
		    die(json_encode(array('message'=>'Sorry! We are unable to connect database using the given credentials')));
		}
	} 
	...
	if (isset($_POST['adduser'])) {
		...
		if ($email == $_SESSION['adminemail']) {
		    ...
		    $stmt = $dbc->prepare("
				...
				UNLOCK TABLES;
		    	");
			   	$stmt->execute(array('fname'=>$fname,'pass'=>$pass,'email'=>$email,'today'=>date('Y-m-d H:i:s')));
			   	echo json_encode(array('message'=>'success'));
